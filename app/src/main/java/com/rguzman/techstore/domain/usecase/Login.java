...
public class Login extends UseCase<Login.Parameters, User> {
    ...
    @Override
    public void execute(boolean forceUpdate, Parameters params, Callback<User> callback) {
        this.userRepository.login(params.email, params.password, new Callback<User>() {
            @Override
